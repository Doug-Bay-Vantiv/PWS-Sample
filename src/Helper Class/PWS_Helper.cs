...
namespace CertSuiteTool.Helper_Class
{
    class PWS_Helper
    {
        ...
        */
        ...
        private static DebitInstrumentType debitInstrumentType()
        {
            ...
            if (target.CboAccountType.Text.Length > 0)
            {
            ...
            dit.PinData.encryptiontype = (EncryptionType)target.CboPINDataEncryptionType.SelectedItem; //EncryptionType.DUKPT;//DUKPT (default), Voltage
            dit.PinData.key = target.TxtPINDataKeySerialNum.Text;//The key used to encrypt the PIN data. This field is not sent as part of the request if the encryption-type is Voltage. Voltage Encryption Transfer data (Encrypted Symmetric Key). This field is not sent as part of the request if the encryption-type is DUKPT.
            dit.PinData.Value = target.TxtTxtPINDataPINBlock.Text; 
            //dit.VoucherNumber = "";
        ...
        private static GiftInstrumentType giftInstrumentType()
        {
            ...
            if (target.CboCreditType.Text == "CardKeyed")
            {
                ...
                if (target.ChkUseToken.Checked)
                {
                    ...
                    if (target.TxtTokenId.Text.Length > 0)
                        git.CardKeyed.Token.tokenId = target.TxtTokenId.Text;
                    if (target.TxtTokenValue.Text.Length > 0)
        ...
        public static ResponseDetails authorizeRequest()
        {
            ...
            {
                TaxAmountType t = new TaxAmountType();
                t.Taxable = target.ChkTaxable.Checked;//It specifies if the transaction amount is taxable or not.
                t.TaxableSpecified = true;
                t.TaxAmount = new AmountType();
                t.TaxAmount.currency = (ISO4217CurrencyCodeType)target.CboCurrencyCodeType.SelectedItem;
                t.TaxAmount.currencySpecified = true;
                t.TaxAmount.Value = Convert.ToDecimal(target.TxtSalesTaxAmount.Text);//It represents the amount that is added as tax to the total amount in the transaction.
                t.TaxExempt = target.ChkTaxExempt.Checked;//It specifies if the transaction amount is exempt from tax or not.
                t.TaxExemptSpecified = true;
        ...
        public static ResponseDetails purchaseRequest()
        {
            ...
            p.TokenRequested = target.ChkTokenRequested.Checked;//Boolean value (true, false) to determine if token is returned for the card. The default value is false.	Optional
            p.TokenRequestedSpecified = target.ChkTokenRequested.Checked;
            //a.BillPaymentPayee = billPaymentPayeeType(); //For PIN-less Debit : Bill payment payee details contain values about the payee including payee name, phone and account number payee uses to identify the payer.
            ...
            //Set the object for payment
            ItemsChoiceType3[] ict = new ItemsChoiceType3[2];
            ...
            {
                MessageBox.Show("NEED TO IMPLEMENT TIMEOUT LOGIC");
            }
            catch (Exception ex)
            {
                if (!CheckForFault(ex))
        ...
        public static ResponseDetails captureRequest(ResponseDetails _rd)
        {
            ...
            //Set the object for payment
            ItemsChoiceType5[] ict = new ItemsChoiceType5[2];
            c.Items = new object[2];
            if (target.CboPaymentInstrument.Text == "Credit")
            {
                c.Items[0] = creditInstrumentType();
                ict[0] = ItemsChoiceType5.Credit;
            }
            else if (target.CboPaymentInstrument.Text == "Debit")
            {
                MessageBox.Show("Debit transactions do not use the Authorize transaction type. Please use Purchase instead.");
        ...
        public static ResponseDetails cancelRequest(ResponseDetails _rd)
        {
            ...
            {//System
                ...
                can.OriginalTransactionTimestamp = ((TransactionRequestType)(_rd.PWS_TxnSummary.PWSRequest)).TransactionTimestamp;
                can.OriginalTransactionTimestampSpecified = true;
                can.OriginalSystemTraceId = ((TransactionRequestType)(_rd.PWS_TxnSummary.PWSRequest)).systemtraceid;
                can.OriginalSystemTraceIdSpecified = true;
                can.OriginalSequenceNumber = ((TransactionRequestType)(_rd.PWS_TxnSummary.PWSRequest)).Merchant.Software.SequenceNumber;
        ...
        public static ResponseDetails closeBatchRequest()
        {
            CloseBatchRequest cbr = new CloseBatchRequest();

            Random r = new Random();
            cbr.DraftLocatorId = "D" + r.Next(1, 99999999).ToString(); //11 character value.  This field can be used to pass whatever discretionary data the merchant wants to pass.  Examples include employee ID number, invoice numbers, any internal value they use to track transactions.	Optional – only passes thru to reporting on Visa and MasterCard transactions.
            cbr.Merchant = merchantType();
            cbr.TransactionTimestamp = DateTime.Now; //The time of this transaction. Use yyyy-MM- ddThh:mm:ss-SS:SS – Should be in merchants local time zone. Mandatory – should be in merchant’s local time zone.
            int rInt = r.Next(1, 99999); //for ints
            cbr.systemtraceid = rInt; //A conditional ID used to track each transaction. This must be an integer. Required for Raft and Tandem, optional for Litle? Required for Litle on CancelRequest.
            cbr.systemtraceidSpecified = true;
            rInt = r.Next(1, 99999999);
            cbr.merchantrefid = "PWS" + rInt.ToString(); //An optional attribute used by the merchant to identify each transaction. This can be no longer than 16 characters. If the merchant chooses not to use this field it is recommended that you populate this ID with the system-trace-id value.
          
            //Balances
            //cbr.Balances = new BalanceEntry();
            //cbr.Balances.ReturnsAmount = new TotalsAmountType();
            //cbr.Balances.ReturnsAmount.currency = (ISO4217CurrencyCodeType)target.CboCurrencyCodeType.SelectedItem;
            //cbr.Balances.ReturnsAmount.currencySpecified = true;
            //cbr.Balances.ReturnsAmount.Value = Convert.ToDecimal(target.TxtTransactionAmount.Text);

            PWS_TransactionSummary ts = new PWS_TransactionSummary(null, null);
            try
            {
                ts = new PWS_TransactionSummary(cbr, target.PWSClient.CloseBatch(cbr));
                //target.PWSClient.CloseBatch(cbr);
            }
            catch (System.TimeoutException te)
            {
                //Timeout has occured. Check to see if a System Reversal should be initiated. Please note that this can be a endless loop
        ...
        public static ResponseDetails ProcessResponse(PWS_TransactionSummary _ts)
        {
            ...
            try
            {
                if (_ts.Response.GetType().BaseType.Name == "TransactionResponseType")
                {
                ...
                else if (_ts.Response.GetType().BaseType.Name == "BatchResponseType")
                {
                    if (((BatchResponseType)(_ts.Response)).ItemsElementName.Count() > 0)
                    {
                        int idx = 0;
                        while (idx < ((BatchResponseType)(_ts.Response)).Items.Count())
                        {
                            if (((BatchResponseType)(_ts.Response)).ItemsElementName[idx] == ItemsChoiceType.AuthorizationCode)
                                AuthorizationCode = ((BatchResponseType)(_ts.Response)).Items[idx];
                            idx++;
                        }
                    }
                    //Add to CheckListBox
                    ...
                    a.currency = (ISO4217CurrencyCodeType)target.CboCurrencyCodeType.SelectedItem;
                    a.currencySpecified = true;
                    a.Value = Convert.ToDecimal("0.00");
        ...
        public static string ExtractDetailsFromResponse(ResponseDetails _rd)
        {
            ...
            try
            {
                ...
                {
                    if (_rd.PWS_TxnSummary.PWSRequest.GetType().BaseType.Name == "TransactionRequestType")
                    {
                    ...
                    else if (_rd.PWS_TxnSummary.PWSRequest.GetType().BaseType.Name == "BatchRequestType")
                    {
                        BatchRequestType brt = (BatchRequestType)_rd.PWS_TxnSummary.PWSRequest;
                        transactionInformation += "REQUEST\r\n";

                        if (brt.Balances != null)
                        {
                            transactionInformation += "Balances\r\n";
                            if (brt.Balances.ReturnsAmount != null)
                            {
                                transactionInformation += "- ReturnsCount: " + brt.Balances.ReturnsAmount.Value + " " + brt.Balances.ReturnsAmount.currency.ToString() + "\r\n";
                            }
                            if (brt.Balances.ReturnsCount != null)
                            {
                                transactionInformation += "- ReturnsCount: " + brt.Balances.ReturnsCount + "\r\n";
                            }
                            if (brt.Balances.SalesAmount != null)
                            {
                                transactionInformation += "- ReturnsCount: " + brt.Balances.SalesAmount.Value + " " + brt.Balances.SalesAmount.currency.ToString() + "\r\n";
                            }
                            if (brt.Balances.SalesCount != null)
                            {
                                transactionInformation += "- ReturnsCount: " + brt.Balances.SalesCount + "\r\n";
                            }
                        }
                        if (brt.DraftLocatorId != null)
                            transactionInformation += "DraftLocatorId: " + brt.DraftLocatorId + "\r\n";
                        if (brt.Merchant != null)
                            transactionInformation += "Merchant\r\n"
                                + " - CashierNumber: " + brt.Merchant.CashierNumber + "\r\n"
                                + " - ChainCode: " + brt.Merchant.ChainCode + "\r\n"
                                + " - ClerkNumber: " + brt.Merchant.ClerkNumber + "\r\n"
                                + " - DivisionNumber: " + brt.Merchant.DivisionNumber + "\r\n"
                                + " - LaneNumber: " + brt.Merchant.LaneNumber + "\r\n"
                                + " - MerchantId: " + brt.Merchant.MerchantId + "\r\n"
                                + " - MerchantName: " + brt.Merchant.MerchantName + "\r\n"
                                //+ " - Mobile: " + trt.Merchant.Mobile + "\r\n"
                                + " - NetworkRouting: " + brt.Merchant.NetworkRouting + "\r\n"
                                //+ " - Software: " + trt.Merchant.Software + "\r\n"
                                + " - StoreNumber: " + brt.Merchant.StoreNumber + "\r\n"
                                //+ " - Terminal: " + trt.Merchant.Terminal + "\r\n"
                                ;
                        if (brt.merchantrefid != null)
                            transactionInformation += "merchantrefid: " + brt.merchantrefid + "\r\n";
                        if (brt.systemtraceid != null)
                            transactionInformation += "systemtraceid: " + brt.systemtraceid + "\r\n";
                        if (brt.TransactionTimestamp != null)
                            transactionInformation += "TransactionTimestamp: " + brt.TransactionTimestamp + "\r\n";
                    }
                }
                if (_rd.PWS_TxnSummary.Response != null)
                {
                    if (_rd.PWS_TxnSummary.Response.GetType().BaseType.Name == "TransactionResponseType")
                    {
                    ...
                    else if (_rd.PWS_TxnSummary.Response.GetType().BaseType.Name == "BatchResponseType")
                    {
                        BatchResponseType brt = (BatchResponseType)_rd.PWS_TxnSummary.Response;
                        transactionInformation += "RESPONSE\r\n";
                        
                        if (brt.Items != null && brt.Items.Count() > 0)
                        {
                            transactionInformation += "Items\r\n";
                            if (((BatchResponseType)(_rd.PWS_TxnSummary.Response)).Items.Count() > 0)
                            {
                                int idx = 0;
                                while (idx < ((BatchResponseType)(_rd.PWS_TxnSummary.Response)).Items.Count())
                                {
                                    transactionInformation += " -" + brt.ItemsElementName[idx] + ": " + brt.Items[idx] + "\r\n";
                                    idx++;
                                }
                            }
                        }
                        if (brt.keyValuePair != null && brt.keyValuePair.Count() > 0)
                        {
                            transactionInformation += "Key Value Pair\r\n";
                            foreach (KeyValuePair kvp in brt.keyValuePair)
                            {
                                transactionInformation += " -Key: " + kvp.key + " Value: " + kvp.Value;
                            }
                        }
                        if (brt.merchantrefid != null)
                            transactionInformation += "merchantrefid: " + brt.merchantrefid + "\r\n";
                        if (brt.reportgroup != null)
                            transactionInformation += "reportgroup: " + brt.reportgroup + "\r\n";
                        if (brt.RequestId != null)
                            transactionInformation += "RequestId: " + brt.RequestId + "\r\n";
                        if (brt.Summary != null)
                        {
                            transactionInformation += "Summary\r\n";
                            if (brt.Summary.ReturnsAmount != null)
                            {
                                transactionInformation += "- ReturnsCount: " + brt.Summary.ReturnsAmount.Value + " " + brt.Summary.ReturnsAmount.currency.ToString() + "\r\n";
                            }
                            if (brt.Summary.ReturnsCount != null)
                            {
                                transactionInformation += "- ReturnsCount: " + brt.Summary.ReturnsCount + "\r\n";
                            }
                            if (brt.Summary.SalesAmount != null)
                            {
                                 transactionInformation += "- ReturnsCount: " + brt.Summary.SalesAmount.Value + " " + brt.Summary.SalesAmount.currency.ToString() + "\r\n";
                            }
                            if (brt.Summary.SalesCount != null)
                            {
                                transactionInformation += "- ReturnsCount: " + brt.Summary.SalesCount + "\r\n";
                            }
                        }
                        if (brt.systemtraceid != null)
                            transactionInformation += "systemtraceid: " + brt.systemtraceid + "\r\n";
                        if (brt.TransactionStatus != null)
                            transactionInformation += "Transaction Status: " + brt.TransactionStatus + "\r\n";
                        if (brt.WorkingKey != null)
                            transactionInformation += "Working Key: " + brt.WorkingKey + "\r\n";
                    }
                }
            }
            catch
    ...
    public class PWS_TransactionSummary
    {
        ...
        */
        public Object PWSRequest;
        public Object Response;

        public PWS_TransactionSummary(Object pWSRequest, Object response)
        {
